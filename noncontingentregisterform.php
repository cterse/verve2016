<?php
	...
	*/
		...
		if(isset($_POST['name1']) && isset($_POST['phone1']) && isset($_POST['email1']) && isset($_POST["cname"])){
			//echo "Success";
			$pos = array();
			//print_r($pos);
			//$count = count($pos);
			...
			//0 = NonCont 1 = Cont 
			$sql = mysqli_query($conn,"INSERT INTO $tableName (members,paid,grouptype) VALUES ($count,null,'0')") or die(mysqli_error($conn)." 1");		//1
...
<html>
	<head>
		...
		<script type="text/javascript">
			function goToEvents(){
				location.replace("eventsnoncont.php");
			}
		...
		<div class="row">
			...
			<div class="col-md-8" style="background:#ff9980;margin-top:2em;margin-bottom:2em;border-radius:2em;">
				<?php
					...
					$n = $temp[0];
				?>
				<h2 id="eventTitle" name="eventTitle" style="margin-left:2em;"><?php echo $n;?></h2>

				<form role="form" autocomplete="off" onsubmit="return validateForm()" action="noncontingentregisterform.php?eventid=<?php echo $eventid;?>" method="post" style="margin:2em;">
					<?php
						//$conn = mysqli_connect("localhost","wasiq","root123","college_db");
					...
					?>
						...
						<div class="form-group">
  							<label for="sel1">College:&nbsp;<span id='req_span_col' name='req_span_col' class='fa fa-asterisk req_spans' style='color:red;'></span></label>
  							<select class="form-control" id="cname" name="cname">
							    ...
							    <option>IIT</option>-->
							    <?php
									    $sql2 = mysqli_query($conn,"SELECT cname FROM collegecode WHERE 1") or die(mysqli_error($conn));
										while ($options = mysqli_fetch_row($sql2)) {
											$optionList = "<option>".$options[0]."</option>";
											echo $optionList;
										}
									?>
  							</select>
						...
						<button type="submit" style="margin-left:44%;" class="btn btn-success btn-lg" >Register</button>
						<button type="button" class="btn btn-warning btn-lg" onclick="goToEvents()" title="Return to Events" style="margin-left:30%;">Cancel</button>
				</form>
			</div>
		</div>
	</BODY>
